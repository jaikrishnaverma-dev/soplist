        ...
        <form action="" id="po-form">
            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="billing_type" class="control-label text-info">Billing type</label>
                            <select id="billing_type" name="is_tax_igst" class="custom-select ">
                                <option value="0" <?php echo (!isset($is_tax_igst))||$is_tax_igst!="1"?'selected':''?>>Under State</option>
                                <option value="1" <?php echo isset($is_tax_igst)&&$is_tax_igst=="1"?'selected':''?> >Intra State</option>
                            </select>
                        </div>
                    </div>
                ...
                <fieldset>
                    ...
                    <div class="row justify-content-center align-items-end">
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                <label for="qty" class="control-label">HSN</label>
                                <input disabled type="text" step="any" class="form-control rounded-0" id="hsn">
                            </div>
                        </div>
                        <div class="col-md-1">
                            <div class="form-group">
                                <label for="qty" class="control-label">GST %</label>
                                <input disabled type="number" step="any" class="form-control rounded-0" id="gst_percent">
                            </div>
                        </div>
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <thead>
                        <tr class="text-light bg-navy">
                            ...
                            <th class="text-center py-1 px-2">HSN</th>
                            <th class="text-center py-1 px-2 text-nowrap">GST %</th>
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td class="py-1 px-2 text-center qty">
                                        ...
                                        <input type="hidden" name="hsn[]" value="<?php echo $row['hsn']; ?>">
                                        <input type="hidden" name="gst_percent[]" value="<?php echo $row['gst_percent']; ?>">
                                    ...
                                    <td class="py-1 px-2 text-right hsn">
                                        <?php echo $row['hsn']; ?>
                                    </td>
                                    <td class="py-1 px-2 text-right gst_percent">
                                        <?php echo floatval($row['gst_percent']); ?>
                                    </td>
                    ...
                    <tfoot>
                        ...
                        <tr>
                            <th class="text-right py-1 px-2" colspan="7">Discount <input style="width:40px !important" name="discount_perc" class='' type="number" min="0" max="100" value="<?php echo isset($discount_perc) ? $discount_perc : 0 ?>">%
...
<script>
    ...
    $(function() {
        ...
        $("#item_id").change(function() {
            ...
            if (items?.[supplier_id]?.[item_id]) {
                const item = items[supplier_id][item_id]
                const cost = parseFloat(item.cost)
                const gst_percent = parseFloat(item.gst_percent)
                const hsn = item.hsn
                $("#hsn").val(hsn)
        ...
        $('#supplier_id').change(function() {
            ...
            if (!!items[supplier_id]) {
                ...
                var list_item = new Promise(resolve => {
                    Object.entries(items[supplier_id]).map(function([k,item],i) {
                        ...
                        if(i==0){
                            $("#hsn").val(item.hsn)
                            $("#gst_percent").val(item.gst_percent)
                        }
                    })
                    resolve()
                })
        ...
        $('#add_to_list').click(function() {
            ...
            tr.find('[name="hsn[]"').val(hsn)
            tr.find('[name="gst_percent[]"').val(gst_percent)
            ...
            tr.find('.hsn').text(hsn);
            tr.find('.gst_percent').text(parseFloat(gst_percent).toLocaleString('en-US'))
    ...
    function calc() {
        ...
        if (istypeIGST) {
            ...
            $("#total_gst_distribution_label").text("Total (SGST + CGST)");
        }

        $('table#list tbody input[name="total[]"]').each(function(index) {
            var itemTotalValue = parseFloat($(this).val());
            sub_total += itemTotalValue;
            // Find the corresponding gst_percent input by index
            var gst_string = $('table#list tbody input[name="gst_percent[]"]').eq(index).val();
            console.log({gst_string})
            var gstValue = parseFloat(gst_string ? gst_string : 0);
            // Calculate tax based on the GST percent and add it to the total tax
            tax += (gstValue * (itemTotalValue / 100));
        ...
        var discount_perc = $('[name="discount_perc"]').val()
        discount_perc = !isNaN(parseFloat(discount_perc)) ? discount_perc : 0
        var discount = sub_total * (parseFloat(discount_perc) / 100)
        sub_total = sub_total - discount;

        // var tax = sub_total * (parseFloat($('[name="tax_perc"]').val()) / 100)
  
        tax = tax - (tax * (parseFloat(discount_perc) / 100))
        ...
        const v= $('[name="tax"]').val()
        console.log({tt:v,tax});
