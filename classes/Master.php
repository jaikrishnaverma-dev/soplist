...
class Master extends DBConnection
{
	...
	function save_po()
	{
		...
		if ($save) {
			...
			foreach ($item_id as $k => $v) {
				if (!empty($data)) $data .= ", ";
				$data .= "('{$po_id}','{$v}','{$qty[$k]}','{$price[$k]}','{$unit[$k]}','{$total[$k]}','{$hsn[$k]}','{$gst_percent[$k]}')";
			}
			if (!empty($data)) {
				$this->conn->query("DELETE FROM `po_items` where po_id = '{$po_id}'");
				$save = $this->conn->query("INSERT INTO `po_items` (`po_id`,`item_id`,`quantity`,`price`,`unit`,`total`,`hsn`,`gst_percent`) VALUES {$data}");
				if (!$save) {
					$resp['status'] = 'failed';
					if (empty($id)) {
					...
					$resp['sql'] = "INSERT INTO `po_items` (`po_id`,`item_id`,`quantity`,`price`,`unit`,`total`,`hsn`,`gst_percent`) VALUES {$data}";
				}
			}

			//jdskdgf
			unset($_POST['is_tax_igst']);
			unset($_POST['po_code']);
			$_POST['form_id'] = $po_id;
			$_POST['po_id'] = $po_id;
			$_POST['from_order'] = 1;
			$_POST['oqty'] = $_POST['qty'];
			$this->save_receiving_after_purchase();
		} else {
			$resp['status'] = 'failed';
	...
	function save_receiving_after_purchase()
	{
		extract($_POST);
		$data = "";
		foreach ($_POST as $k => $v) {
		...
		if ($save) {
			...
			if (isset($_POST['hsn'])) {
				...
				foreach ($item_id as $k => $v) {
					if (!empty($data)) $data .= ", ";
					$sql = "INSERT INTO stock_list (`item_id`,`quantity`,`price`,`unit`,`total`,`type`,`hsn`,`gst_percent`) VALUES ('{$v}','{$qty[$k]}','{$price[$k]}','{$unit[$k]}','{$total[$k]}','1','{$hsn[$k]}','{$gst_percent[$k]}')";
					$this->conn->query($sql);
	...
	function save_sale()
	{
		...
		if ($save) {
			...
			foreach ($item_id as $k => $v) {
				$sql = "INSERT INTO `stock_list` set item_id='{$v}', `quantity` = '{$qty[$k]}', `unit` = '{$unit[$k]}', `price` = '{$price[$k]}', `total` = '{$total[$k]}', `type` = 2 , `hsn` = '{$hsn[$k]}' , `gst_percent` = '{$gst_percent[$k]}' ";
				$save = $this->conn->query($sql);
				if ($save) {
					$sids[] = $this->conn->insert_id;
				}
